...
class StockApiController extends BaseApiController {
	...
	public function ImportProductsList(Request $request, Response $response, array $args) {
		User::checkPermission($request, User::PERMISSION_STOCK_EDIT);
		try {
			$uploadedFiles = $request->getUploadedFiles();
			$excelFile = $uploadedFiles['excel'] ?? null;
			...
			if ($excelFile->getError() !== UPLOAD_ERR_OK) {
				throw new \Exception('上传错误: ' . $excelFile->getError());
			}

			// 验证文件类型
			$clientFilename = $excelFile->getClientFilename();
			$extension = pathinfo($clientFilename, PATHINFO_EXTENSION);
			if (strtolower($extension) !== 'xlsx') {
				throw new \Exception('仅支持 .xlsx 文件');
			}

			// 创建上传目录
			$uploadDir = $this->getFilesService()->GetFilePath('excel', '');
			if (!is_dir($uploadDir)) {
			...
			$list = ExcelService::parseExcel($targetPath);
			...
			if ($list) {
				...
				foreach ($fieldList as $v) {
					$fieldMap[$v['name']] = $v['id'];
				}

				// 修改产品表
				$sql_product = 'INSERT INTO products (id, name,location_id,qu_id_purchase,qu_id_stock)
					VALUES (:id, :name,0,0,0)
					ON CONFLICT(id) DO UPDATE SET name = :name';
				$now = date('Y-m-d H:i:s');

				$sql_field = 'INSERT INTO userfield_values (field_id, object_id,value,row_created_timestamp)
				VALUES (:field_id, :object_id, :value, :now)
				ON CONFLICT(field_id,object_id) DO UPDATE SET value = :value';

				foreach ($list as $item) {
					$this->getDatabaseService()->ExecuteDbStatement($sql_product, ['id' => $item['product_id'], 'name' => $item['product_name']]);
					$product_id = $item['product_id'];
					unset($item['product_id'], $item['product_name']);
					...
					foreach ($item as $k => $v) {
						$this->getDatabaseService()->ExecuteDbStatement($sql_field, ['field_id' => $fieldMap[$k], 'object_id' => $product_id, 'value' => $v, 'now' => $now]);
					}
				}
			}
