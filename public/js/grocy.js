...
__t = function (text, ...placeholderValues) {
	...
	if (Grocy.Mode === "dev") {
		...
		if (
			...
			!Grocy.LocalizationStrings.messages[""].hasOwnProperty(text2)
		) {
	...
	try {
		return sprintf(
			Grocy.Translator.__(text, ...placeholderValues),
			...placeholderValues
		);
	} catch (e) {
...
__n = function (number, singularForm, pluralForm, isQu = false) {
	...
	if (Grocy.Mode === "dev") {
		...
		if (
			Grocy.LocalizationStrings &&
			!Grocy.LocalizationStrings.messages[""].hasOwnProperty(
				singularForm2
			)
		) {
	...
	if (isQu) {
		return sprintf(
			Grocy.TranslatorQu.n__(singularForm, pluralForm, number, number),
			number.toLocaleString()
		);
	} else {
		return sprintf(
			Grocy.Translator.n__(singularForm, pluralForm, number, number),
			number.toLocaleString()
		);
	}
};
...
RefreshContextualTimeago = function (rootSelector = "#page-content") {
	$(rootSelector + " time.timeago").each(function () {
		...
		var isToday =
			timestamp &&
			timestamp.substring(0, 10) == moment().format("YYYY-MM-DD");
...
Grocy.FrontendHelpers.ShowGenericError = function (message, exception) {
	toastr.error(
		__t(message) + "<br><br>" + __t("Click to show technical details"),
		"",
		{
			onclick: function () {
				...
				if (
					...
					exception !== null &&
					exception.hasOwnProperty("error_message")
				) {
...
Grocy.FrontendHelpers.SaveUserSetting = function (
	settingsKey,
	value,
	force = false
) {
...
Grocy.FrontendHelpers.DeleteUserSetting = function (
	settingsKey,
		...
		function (xhr) {
			if (xhr.statusText) {
				Grocy.FrontendHelpers.ShowGenericError(
					"Error while deleting, please retry",
					xhr.response
				);
			}
		}
		...
		if (webhook.json) {
			$.ajax(webhook.hook, {
				...
				if (!hasAlreadyFailed) {
					...
					Grocy.FrontendHelpers.ShowGenericError(
						__t("Error while executing WebHook", {
							status: status,
			...
			$.post(webhook.hook, data).fail(function (
				req,
				status,
				errorThrown
			) {
...
$(document).on("change", ".user-setting-control", function () {
	...
	if (
		typeof element.attr("type") !== typeof undefined &&
		element.attr("type") !== false
	) {
...
$("input.custom-file-input").on("change", function () {
	$(this)
		...
		.html(GetFileNameFromPath($(this).val()));
});
...
ResizeResponsiveEmbeds = function () {
	...
	$("embed.embed-responsive:not(.resize-done)")
		.attr("height", maxHeight.toString() + "px")
		.addClass("resize-done");
};
	...
	.each(function (index, child) {
		new ResizeObserver(function () {
			window.parent.postMessage(
				WindowMessageBag("ResizeResponsiveEmbeds"),
				Grocy.BaseUrl
			);
		}).observe(child);
	});
...
function RefreshLocaleNumberDisplay(rootSelector = "#page-content") {
	$(
		rootSelector +
			" .locale-number.locale-number-currency:not('.number-parsing-done')"
	).each(function () {
		var element = $(this);
	...
	$(
		rootSelector +
			" .locale-number.locale-number-quantity-amount:not('.number-parsing-done')"
	).each(function () {
		var element = $(this);
		var text = element.text();
		if (!text || Number.isNaN(text)) {
		...
		element.text(
			value.toLocaleString(undefined, {
				...
				maximumFractionDigits:
					Grocy.UserSettings.stock_decimal_places_amounts,
			})
		);
		element.addClass("number-parsing-done");
	});
	...
	$(
		rootSelector +
			" .locale-number.locale-number-generic:not('.number-parsing-done')"
	).each(function () {
		var element = $(this);
		var text = element.text();
		if (!text || Number.isNaN(text)) {
...
function RefreshLocaleNumberInput(rootSelector = "#page-content") {
	$(rootSelector + " .locale-number-input.locale-number-currency").each(
		function () {
			var element = $(this);
			var value = element.val();
			...
			element.val(
				Number.parseFloat(value).toLocaleString("en", {
					minimumFractionDigits:
						Grocy.UserSettings.stock_decimal_places_prices_input,
					maximumFractionDigits:
						Grocy.UserSettings.stock_decimal_places_prices_input,
					useGrouping: false,
				})
	...
	$(
		rootSelector + " .locale-number-input.locale-number-quantity-amount"
	).each(function () {
		var element = $(this);
		var value = element.val();
		if (!value || Number.isNaN(value)) {
	...
	$(rootSelector + " .locale-number-input.locale-number-generic").each(
		function () {
			var element = $(this);
			var value = element.val();
			if (!value || Number.isNaN(value)) {
...
if (GetUriParam("embedded")) {
	$("body").append(
		'<div class="fixed-top" style="left: unset;"> \
...
$(document).on("click", ".close-last-modal-button", function () {
	window.parent.postMessage(
		WindowMessageBag("CloseLastModal"),
		Grocy.BaseUrl
	);
});
...
$(window).on("message", function (e) {
	...
	if (data.Message == "ShowSuccessMessage") {
		...
		$(".modal:visible")
			.not(".custom-escape-key-handling")
			.last()
			.modal("hide");
	...
	.each(function () {
		...
		if (base.contains("?")) {
			$(this).attr(
				...
				base +
					"&returnto" +
					encodeURIComponent(Grocy.CurrentUrlRelative)
			);
		} else {
			$(this).attr(
				"href",
...
$(".dropdown-item")
	...
	.on("click", function (e) {
		if (
			$(e.target).is("div.form-check") ||
			$(e.target).is("div.dropdown-item")
		) {
			$(e.target).find("input[type=checkbox]").click();
		}
	});

$('[data-toggle="tooltip"][data-html="true"]').on(
	"shown.bs.tooltip",
	function () {
